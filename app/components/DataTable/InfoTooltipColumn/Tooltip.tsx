...
import { formattedDate, getSafeValue } from "~/utils";
        ...
        <Stack spacing={0.5} sx={{ p: 0.5 }}>
          ...
          {updatedAt && (
            <TooltipItem
              ...
              value={formattedDate(updatedAt)}
            />
          )}
          {createdBy && (
            <TooltipItem
              label={<FormattedMessage {...messages.createdBy} />}
              value={getSafeValue(createdBy)}
            />
          )}
          {updatedBy && (
            <TooltipItem
              label={<FormattedMessage {...messages.updatedBy} />}
              value={getSafeValue(updatedBy)}
            />
          )}
